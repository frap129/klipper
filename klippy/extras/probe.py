    ...
    def run_probe(self, gcmd):
        ...
        while len(positions) < sample_count:
            ...
            if max(z_positions) - min(z_positions) > samples_tolerance:
                if retries >= 10: # should be samples_retries
                    raise gcmd.error("Probe samples exceed samples_tolerance, probe failed")
                gcmd.respond_info("Probe samples exceed tolerance. Retrying...")
                # 2023/12/13 probe retry 
                # Chengwei
                last_probe_failed = True
                retries += 1
                positions = []
                self.probe_count += 1
                #gcode.respond_info('Probe failed, probe count is ' + str(self.probe_count))
            # Retract
